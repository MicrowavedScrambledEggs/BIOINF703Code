...
G <- readGraphFromFile("labNet.txt", returnNames = TRUE)
```

## Plotting G

```{r plottingG}
plot.network(G, mode = "kamadakawai", edge.col = rgb(red = 0, blue = 0, green = 0, alpha = 0.05))
```
...
From the above histograms we can see that ER is a poor model for G. The smaller variance in degree compared to G is reflected in the degree centralization histogram, with the value for G falling considerably outside the distribution of the ER values. The Mean local clustering coefficient histogram shows that G's neighbourhoods are considerably more connected than those of a ER network. As can be expected from the higher frequency of nodes with degree lower than the mean, the mean betweenness of G is higher than the mean betweenness of all the ER networks, as the nodes with lower degree will typically have less shortest paths between them.
...
Building a network by sampling from the degree distribution of G provides a model that is at least better than ER. The mean betweenness of these random graphs are all higher than that of G. This is possibly due to how G, being a natural network, would likely have 'hubs' of interconnected nodes and a few nodes that connect the hubs to each other. The hub connecting nodes would have high betweenness but the mean value would be reduced by nodes in the hubs with lower betweenness. As a random graph would not likely share this structure nodes would more likely be on shortest paths between other nodes.
...
The mean local clustering coefficient being higher than these random graphs also potentially indicates a 'connected intraconnected hubs' structure. Nodes in these hubs would have high local clustering coefficient. Random graphs without a bias towards connected neighbourhoods would have lower local clustering coefficient.
